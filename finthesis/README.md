...
* Постављају се одговарајуће маргине и величина фонта. За разлику од правилника, не користи се *Times New Roman* фонт, већ [*Computer Modern*](https://cm-unicode.sourceforge.io). Подешен је српски језик и ћирилично писмо.
...
* bachelor --- документ је завршни рад
* **diploma** --- документ је дипломски рад
* master --- документ је мастер рад
* hints --- уколико није задата нека од потребних информација за насловне стране и страну са апстрактом, на одговарајућем месту се појављује помоћни текст који указује на тачну наребду коју треба користити
* **nohints** --- искључује приказ помоћног текста на насловним странама и страном са апстрактом уколико нека од обавезних наредби није коришћена
* **usehyperref** --- користи се [hyperref](https://ctan.org/pkg/hyperref) пакет
* nousehyperref --- искључује коришћење [hyperref](https://ctan.org/pkg/hyperref) пакета
* **pdfmetadata** --- уколико је задата опција usehyperref, попуњавају се одговарајућа поља мета-података излазне pdf датотеке
* nopdfmetadata --- искључује попуњавање поља мета-података излазне pdf датотеке
* pdfcoursesubject --- уколико су задате опције usehyperref и pdfmetadata, садржај поља мета-податка излазне pdf датотеке под називом "Subject" попуњава се називом предмета
* **nopdfcoursesubject** --- искључује попуњавање поља мета-податка "Subject" у излазној pdf датотеци
...
* `\title{...}` --- наслов завршног, дипломског или мастер рада
* `\author{...}` --- име, средње слово и презиме аутора (студента)
* `\date{YYYY-MM-DD}` --- датум
* `\studid{...}` --- број индекса
* `\studprog{...}` --- назив студијског програма
* `\module{...}` --- назив модула; ова наредба није обавезна за коришћење
* `\course{...}` --- назив предмета
* `\advisor{...}` --- име ментора са титулом
* `\advisorfull{...}` --- име ментора са титулом и наставним звањем
* `\committee{...}` --- овом наредбом додаје се један члан комисије; може се користити више пута за додавање више чланова комисије
* `\addbibresource{main.bib}` --- увози се датотека `main.bib` као извор информација за литературу
* `\titlepagebib{key}` --- овом наредбом додаје се један ред литературе *на насловној страни*, где је `key` кључ ставке у увезеној .bib датотеци; може се користити више пута за додавање додатне литературе
* `\studentshould{...}` --- управља се садржајем насловне стране која почиње са "*У оквиру овог завршног/дипломског/мастер рада кадидат треба да...*"
* `\thesisapplicationfile{path}` --- приказује скенирану пријаву завршног/дипломског/мастер рада на четвртој насловној страни, где је `path` релативна путања ка слици или pdf датотеци која садржи скенирану пријаву
...
* `\abstracten{...}` --- апстракт на енглеском
* `\keywordsen{...}` --- кључне речи на енглеском
* `\abstractsr{...}` --- апстракт на српском
* `\keywordssr{...}` --- кључне речи на српском
...
* `\thesiscase{case}` --- исписује тип рада (завршни, дипломски, мастер), где је `case` одговарајући суфикс добијен применом падежа. На пример, текст "У оквиру овог `\thesiscase{ог}` рада\ldots" претвара се у:

  * "У оквиру овог дипломског рада…" --- уколико је задата опција *diploma* класи
  * "У оквиру овог завршног рада…" --- уколико је задата опција *bachelor* класи
  * "У оквиру овог мастер рада…" --- уколико је задата опција *master* класи

* `\setlstmaxlinedigits{num}` --- информише класу о максималној дужини (по броју цифара) нумерације редова у листинзима, ради извођења правилног поравнања у односу на леву маргину документа; подразумевана вредност параметра `num` је 2, односно подразумева се да се листинзи састоје највише из двоцифреног броја редова.
...
**Решење:** За одређени блок текста могуће је подесити граматичка правила тако да она одговарају страном језику. У ту сврху користи се наредба `\foreignlanguage{language}{text}`. Ова наредба се такође може користити унутар .bib датотеке.
